...
namespace CulinarioAPI.Repositories
{
    public class UserProfileRepository : IUserProfileRepository
    {
        ...
        public async Task<UserProfile> GetUserProfileByIdAsync(int id)
        {
            ...
            try
            {
                var profile = await _context.UserProfiles
                    ...
                    .SingleOrDefaultAsync(p => p.UserProfileId == id);
        ...
        {
            _logger.LogInformation("UpdateUserProfileAsync called for UserProfileId: {UserProfileId}", profile.UserProfileId);

            try
            {
                _context.Entry(profile).State = EntityState.Modified;
                await _context.SaveChangesAsync();
                _logger.LogInformation("UserProfile updated successfully for UserProfileId: {UserProfileId}", profile.UserProfileId);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "An error occurred while updating UserProfile for UserProfileId: {UserProfileId}", profile.UserProfileId);
                throw;
            }
        }
        ...
        public async Task<bool> UserProfileExistsAsync(int id)
        {
            ...
            try
            {
                var exists = await _context.UserProfiles.AnyAsync(p => p.UserProfileId == id);
